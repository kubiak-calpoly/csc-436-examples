...
import dev.csse.kubiak.btdemo.bt.ConnectionResult
import dev.csse.kubiak.btdemo.bt.BTDevice
import dev.csse.kubiak.btdemo.bt.BTMessage
    ...
    return flow.onEach {result ->
        when(result) {
          ...
          is ConnectionResult.TransferSucceeded -> {
            Log.i("BTViewModel", "Transfer Succeeded: $result")
            _state.update { it.copy(
              messages = it.messages + result.message
            ) }
          }
          is ConnectionResult.Error -> {
            Log.e("BTViewModel", "Error: ${result.message}")
            _state.update { it.copy(
              isConnected = false,
              isConnecting = false,
      ...
      .catch { throwable ->
        Log.e("BTViewModel", "Caught exception", throwable)
        controller.closeConnection()
        _state.update { it.copy(
          isConnected = false,
          isConnecting = false,
